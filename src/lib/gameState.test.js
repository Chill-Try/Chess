...
test('给马化腾充 Q 币会优先从兵中随机补足到 4 个，再把当前行棋方所有后变为马', () => {
  const game = new Chess('4k3/8/8/8/8/8/RBBPQP2/4K2Q w - - 0 1')

  const transformedGame = transformCurrentTurnPawnsToKnights(game, () => 0)

  assert.equal(transformedGame.turn(), 'w')
  assert.equal(transformedGame.get('e1')?.type, 'k')
  assert.equal(transformedGame.get('h1')?.type, 'n')
  assert.equal(transformedGame.get('e2')?.type, 'n')
  assert.equal(transformedGame.get('f2')?.type, 'n')
  assert.equal(transformedGame.get('d2')?.type, 'n')
  assert.equal(transformedGame.get('c2')?.type, 'b')
  assert.equal(transformedGame.get('a2')?.type, 'r')
})

test('给马化腾充 Q 币在当前行棋方已有至少 4 个后时只会把这些后变为马', () => {
  const game = new Chess('4k3/8/8/8/8/8/QQQQR3/4K3 w - - 0 1')
  ...
  assert.equal(transformedGame.get('d2')?.type, 'n')
  assert.equal(transformedGame.get('e2')?.type, 'r')
})

test('给马化腾充 Q 币在兵不够时会再从其他可变棋子中补齐', () => {
  const game = new Chess('4k3/8/8/8/8/8/PBQ2P2/4K3 w - - 0 1')
...
test('给马化腾充 Q 币在可变棋子不足时会把所有可变棋子都变为马', () => {
  const game = new Chess('4k3/8/8/8/8/8/PBQ5/4K3 w - - 0 1')
...
test('请陈帅吃肯德基在当前行棋方只剩王兵后时也会把兵变为后', () => {
  const game = new Chess('4k3/8/8/8/8/8/PPP1Q3/4K3 w - - 0 1')

  const transformedGame = transformCurrentTurnNonKingPiecesToQueens(game)

  assert.equal(transformedGame.turn(), 'w')
  assert.equal(transformedGame.get('e1')?.type, 'k')
  assert.equal(transformedGame.get('e2')?.type, 'q')
  assert.equal(transformedGame.get('a2')?.type, 'q')
  assert.equal(transformedGame.get('b2')?.type, 'q')
  assert.equal(transformedGame.get('c2')?.type, 'q')
})
...
test('残局对抗的自定义 FEN 局面在后续走子后仍会保留既有历史', () => {
  const endgameGame = new Chess('4k3/8/8/8/8/8/4Q3/4K3 w - - 0 1')

  const nextGame = cloneGameWithHistory(endgameGame)
  nextGame.move('Qe7#')

  const clonedAfterMove = cloneGameWithHistory(nextGame)
